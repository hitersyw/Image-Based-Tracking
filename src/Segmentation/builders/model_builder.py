...
def download_checkpoints(model_name):
    subprocess.check_output(["python", "./src/Segmentation/utils/get_pretrained_checkpoints.py", "--model=" + model_name])
...
def build_model(model_name, net_input, num_classes, crop_width, crop_height, frontend="ResNet101", is_training=True):
	...
	if "MobileNetV2" == frontend and not os.path.isfile("src/Segmentation/models/mobilenet_v2.ckpt.data-00000-of-00001"):
	    download_checkpoints("MobileNetV2")
	if "InceptionV4" == frontend and not os.path.isfile("src/Segmentation/models/inception_v4.ckpt"):
